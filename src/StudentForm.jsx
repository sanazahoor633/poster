...
const StudentForm = () => {
  ...
  const totalRegistered = courses.reduce((total, { students }) => total + (Number.parseInt(students, 10) || 0), 0)
    ...
    <main className="min-h-screen bg-[#f6f3ed] px-4 py-8 text-[#25312b] sm:px-8 lg:px-14" dir="rtl">
      <section className="mx-auto max-w-7xl">
        ...
        <div className="overflow-hidden rounded-2xl border border-[#d6ddd2] bg-white shadow-sm">
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-right text-sm"><thead className="bg-[#edf3ea] text-[#315847]"><tr><th className="table-head w-16">نمبر</th><th className="table-head">کورس کا نام</th><th className="table-head">انچارج کا نام</th><th className="table-head">رجسٹرڈ طالبات</th><th className="table-head">تاریخ آغاز</th><th className="table-head">تاریخ اختتام</th><th className="table-head">وینیو</th><th className="table-head">عمل</th></tr></thead><tbody>
            {filteredCourses.map((item, index) => <tr key={item.id} className="border-t border-[#edf0eb] transition hover:bg-[#fafcf8]"><td className="table-cell font-semibold text-[#688166]" dir="ltr">{index + 1}</td><td className="table-cell font-semibold text-[#263d33]">{item.course}</td><td className="table-cell">{item.incharge || '—'}</td><td className="table-cell" dir="ltr">{item.students || '—'}</td><td className="table-cell">{item.start || '—'}</td><td className="table-cell">{item.end || '—'}</td><td className="table-cell">{item.venue || '—'}</td><td className="table-cell"><div className="flex gap-2"><button type="button" onClick={() => startEditing(item)} className="rounded-lg border border-[#9bb69b] px-3 py-1 text-xs font-bold text-[#315847] hover:bg-[#e6eee3]">ترمیم</button><button type="button" onClick={() => deleteCourse(item)} className="rounded-lg border border-[#e7b5ad] px-3 py-1 text-xs font-bold text-[#a13e32] hover:bg-[#fff1ee]">حذف</button></div></td></tr>)}
          </tbody></table></div>
        </div>
      ...
      {editingId && <div className="fixed inset-0 z-10 grid place-items-center bg-[#15241d]/45 p-4">
        <form onSubmit={saveCourse} className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl">
          ...
          <div className="mt-6 flex gap-3"><button type="submit" className="rounded-xl bg-[#24483c] px-5 py-2.5 text-sm font-bold text-white hover:bg-[#17362d]">تبدیلیاں محفوظ کریں</button><button type="button" onClick={() => setEditingId(null)} className="rounded-xl border border-[#cbd5c8] px-5 py-2.5 text-sm font-bold text-[#526158]">منسوخ</button></div>
        </form>
      </div>}
    </main>
  )
