      ...
      <section className="mx-auto max-w-7xl">
        ...
        <div className="overflow-hidden rounded-2xl border border-[#d6ddd2] bg-white shadow-sm">
          ...
          <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[980px] text-right text-sm"><thead className="bg-[#edf3ea] text-[#315847]"><tr><th className="table-head w-16">نمبر</th><th className="table-head">کورس کا نام</th><th className="table-head">انچارج کا نام</th><th className="table-head">رجسٹرڈ طالبات</th><th className="table-head">تاریخ آغاز</th><th className="table-head">تاریخ اختتام</th><th className="table-head">وینیو</th><th className="table-head">عمل</th></tr></thead><tbody>
            {filteredCourses.map((item, index) => <tr key={item.id} className="border-t border-[#edf0eb] transition hover:bg-[#fafcf8]"><td className="table-cell font-semibold text-[#688166]" dir="ltr">{index + 1}</td><td className="table-cell font-semibold text-[#263d33]">{item.course}</td><td className="table-cell">{item.incharge || '—'}</td><td className="table-cell" dir="ltr">{item.students || '—'}</td><td className="table-cell">{item.start || '—'}</td><td className="table-cell">{item.end || '—'}</td><td className="table-cell">{item.venue || '—'}</td><td className="table-cell"><div className="flex gap-2"><button type="button" onClick={() => startEditing(item)} className="rounded-lg border border-[#9bb69b] px-3 py-1 text-xs font-bold text-[#315847] hover:bg-[#e6eee3]">Update</button><button type="button" onClick={() => deleteCourse(item)} className="rounded-lg border border-[#e7b5ad] px-3 py-1 text-xs font-bold text-[#a13e32] hover:bg-[#fff1ee]">Delete</button></div></td></tr>)}
          </tbody></table></div>
